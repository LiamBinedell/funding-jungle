...
async function submitForm(e) {
    ...
    // Get first name and last name from the events form
    const eventsFormData = await getEventsFormData();

    // Get the current date
    const currentDate = new Date().toLocaleDateString();

    saveApplication(name, company, loggedInFundingManager, message, eventsFormData.firstName, eventsFormData.lastName, eventsFormData.applicantEmail, currentDate);
...
async function getEventsFormData() {
    const dbRef = ref(database);
    const snapshot = await get(child(dbRef, 'eventsform'));
    let firstName = '';
    let lastName = '';
    let applicantEmail = '';
